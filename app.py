...
STRIP_TOKENS_PATTERN = re.compile(
    r"\b(2160p|1440p|1080p|720p|480p|360p|240p|x264|x265|hevc|hdrip|webrip|web[- ]?dl|bluray|dvdrip|brrip|10bit|8bit|esubs?|dual[- ]?audio|amzn|nf|hindi|english|tamil|telugu|malayalam|fhd|uhd|hd|movies|movie|films|film|org|uncut|aac|ac3|dts|mkv|mp4|avi|moviesmod|moviesflix|bolly4u|filmyzilla|vegamovies|hollywood|bollywood)\b|[@&#+*]",
    re.IGNORECASE,
)
...
def extract_movie_key(file_name: str, caption: str = "") -> str:
    source = (file_name or caption or "unknown_movie").lower()

    # File extension remove karo
    source = re.sub(r'\.(mkv|mp4|avi|mov|webm|m4v|flv|wmv|ts|m2ts)$', '', source)

    # Brackets aur separators clean karo
    source = source.replace("_", " ").replace(".", " ")
    source = re.sub(r'[\[\](){}]', ' ', source)

    # Technical tokens strip karo
    source = STRIP_TOKENS_PATTERN.sub(" ", source)
    source = re.sub(r"\b\d{3,4}p\b", " ", source)

    # --- MAIN FIX: Year dhundho, uske baad sab ignore karo ---
    # Sirf "Title + Year" movie_key banega
    # "jersey 2019 uncut or aac" → "jersey 2019"
    # "jersey 2019 uncut or"     → "jersey 2019" (same key!)
    year_match = re.search(r'\b(19|20)\d{2}\b', source)
    if year_match:
        year = year_match.group(0)
        title = source[:year_match.start()]
        title = re.sub(r'\s+', ' ', title).strip()
        if title:
            return f"{title} {year}"[:120]
        return year

    # Year na mile toh puri cleaning karo
